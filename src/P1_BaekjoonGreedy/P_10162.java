 ...
 * 각각의 횟수 사이에는 빈 칸을 둔다. 해당 버튼을 누르지 않는 경우에는 숫자 0을 출력해야 한다.
...
public class P_10162 {
    public static void main(String[] args) {
        ...
        int[] buttonMinute = {300, 60, 10};
        int[] buttonCount = {0, 0, 0};
        if (time % 10 != 0) {
            System.out.println(-1);
        } else {
            while (time != 0) {
                if (time / buttonMinute[0] != 0) {
                    time = time - buttonMinute[0];
                    buttonCount[0]++;
                } else if (time / buttonMinute[1] != 0) {
                    time = time - buttonMinute[1];
                    buttonCount[1]++;
                } else if (time / buttonMinute[2] != 0) {
                    time = time - buttonMinute[2];
                    buttonCount[2]++;
                }
            }
            System.out.println(buttonCount[0] + " " + buttonCount[1] + " " + buttonCount[2]);
        }
    }
}
